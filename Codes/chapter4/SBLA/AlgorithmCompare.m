...
SNR_range = 25;
BER_Total_avg = zeros(length(SNR_range),5);
MSE_Total = zeros(length(SNR_range),5);
for snr_index = 1:length(SNR_range)
    ...
    for algorithmName = ALGO'
        ...
        elseif(strcmp(algorithmName,'Fischer'))
           [loadedBit,loadedPower]=...
            BitPowerLoadingAlgorithm.Fischer(...
                H,SNR,totalPower,targetRate);
        elseif(strcmp(algorithmName,'SBLA'))
           [loadedBit,loadedPower]=...
            BitPowerLoadingAlgorithm.SBLA(...
                H,SNR,totalPower,targetRate);
        elseif(strcmp(algorithmName,'ImprovedSBLA'))
           [loadedBit,loadedPower]=...
            BitPowerLoadingAlgorithm.ImprovedSBLA(...
                H,SNR,totalPower,targetRate); 
        else
        ...
        filename = ['./resTarget_42/Result_',char(algorithmName),'_SNR_',num2str(SNR),'dB.mat'];
        save(filename,'SNR');
        ...
        for channelEstiMethod = cem' 
            ...
            elseif(strcmp(algorithmName,'SBLA'))
                BER_Total_avg(snr_index,4)=BER_avg;
            elseif(strcmp(algorithmName,'ImprovedSBLA'))
                BER_Total_avg(snr_index,5)=BER_avg;
            else
                error('no such algorithmName');
...
save('./resTarget_42/BER_MSE_Total.mat','BER_Total_avg','MSE_Total');